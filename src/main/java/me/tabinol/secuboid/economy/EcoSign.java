...
public class EcoSign {
	...
	public EcoSign(ILand land, Player player) throws SignException {
		...
		Secuboid.getThisPlugin().iLog().write("SignToCreate: PlayerYaw: " + player.getLocation().getYaw() +
	...
	public void createSignForSale(double price) throws SignException {
		...
		lines[0] = ChatColor.GREEN
				+ Secuboid.getThisPlugin().iLanguage().getMessage("SIGN.SALE.FORSALE");
		lines[1] = ChatColor.GREEN + land.getName();
		lines[2] = "";
		lines[3] = ChatColor.BLUE + Secuboid.getThisPlugin().iPlayerMoney().toFormat(price);
	...
	public void createSignForRent(double price, int renew,
		...
		if (tenantName != null) {
			lines[0] = ChatColor.RED
					+ Secuboid.getThisPlugin().iLanguage().getMessage("SIGN.RENT.RENTED");
			lines[1] = ChatColor.RED + tenantName;
		} else {
			lines[0] = ChatColor.GREEN
					+ Secuboid.getThisPlugin().iLanguage().getMessage("SIGN.RENT.FORRENT");
			lines[1] = ChatColor.GREEN + land.getName();
		}

		if (autoRenew) {
			lines[2] = ChatColor.BLUE
					+ Secuboid.getThisPlugin().iLanguage().getMessage("SIGN.RENT.AUTORENEW");
		} else {
			lines[2] = "";
		}
	...
	public void createSign(String[] lines) throws SignException {
		...
		if (Secuboid.getThisPlugin().iLands().getLand(location) != land) {
			throw new SignException();
		}
